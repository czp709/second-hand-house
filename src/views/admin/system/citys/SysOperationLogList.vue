  ...
  <div class="Sidebar">
    ...
    <el-table :data="tableData2" id="out-table">
        <el-table-column align="center" header-align="center" label="图片" >
            ...
            <img :src="row.imgurl" v-if="row.imgurl" width="40" height="40" @click="open($index)" class="head_pic"/>
            </template>
        </el-table-column>

        <el-table-column align="center" header-align="center" label="甲方" >
            <template slot-scope="{row}">
            <span>{{row.nameone}}</span>
            </template>
        </el-table-column>

        <el-table-column align="center" header-align="center" label="乙方" >
            <template slot-scope="{row}">
            <span>{{row.nametwo}}</span>
            </template>
        </el-table-column>

        <el-table-column align="center" header-align="center" label="签约时间">
            <template slot-scope="{row}">
            <span>{{row.date}}</span>
            </template>
        </el-table-column>
    ...
    export default {
        ...
        data() {
         ...
         methods: {
            ...
            open(e) {
                var that = this
                this.$alert('<img src='+that.tableData[e].imgurl+' width="200" class="head_pic"/>', '图片预览', {
            ...
            getimg(){
                ...
                for(let i=0;i<this.tableData.length;i++){
                    if(that.tableData[i].imgurl){
                        if(that.tableData[i].imgurl.includes('cloud://zufang-4gej561k8ccf648d')){
                            img.push(that.tableData[i].imgurl)
                        }
                    }
                }
                app
                ...
                .then((res) => {
                  console.log(res)
                    for(let i=0;i<that.tableData.length;i++){
                        if(that.tableData[i].imgurl){
                            that.tableData[i].imgurl = res.fileList[i].download_url
                        }
                        else{
                            continue
                        }
        ...
        created: function () {
            ...
            var that = this;
            const query="db.collection(\"heyue\").limit(342).get()";
            const querydata={
                env:'zufang-4gej561k8ccf648d',
                query:query
            };
